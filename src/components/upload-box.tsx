...
export function UploadBox({ onUpload, isUploading = false }: UploadBoxProps) {
        ...
        <div className="space-y-2">
          ...
          >
            ...
            <div className="text-center flex-1 min-w-0">
              ...
              {localFiles.length > 0 && (
                <p className="text-xs text-muted-foreground">
                  Tổng: {(localFiles.reduce((sum, f) => sum + f.size, 0) / 1024).toFixed(2)} KB
                </p>
              )}
            </div>
            {localFiles.length > 0 && (
          ...
          <div className="mt-2 rounded-lg border bg-muted/50 p-3 md:p-4 overflow-hidden">
            <div className="flex items-center justify-between mb-2 gap-2">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <span className="text-sm font-medium truncate">Văn bản đã nhập</span>
              </div>
              <Button
                variant="ghost"
                size="icon"
        ...
        >
          {isUploading ? (
            ...
            <>
              ...
              {hasContent ? 'Tạo mã chuyển file' : 'Vui lòng nhập văn bản hoặc chọn file'}
            </>
          )}
        </Button>
      </CardContent>
